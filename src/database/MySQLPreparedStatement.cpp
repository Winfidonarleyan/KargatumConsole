...
Warhead::Database::MySQLPreparedStatement::MySQLPreparedStatement(MySQLStmt* stmt, std::string_view queryString) :
    _mysqlStmt(stmt),
...
Warhead::Database::MySQLPreparedStatement::~MySQLPreparedStatement()
{
    ClearParameters();
...
void Warhead::Database::MySQLPreparedStatement::BindParameters(PreparedStatement* stmt)
{
...
void Warhead::Database::MySQLPreparedStatement::ClearParameters()
{
...
//- Bind on mysql level
void Warhead::Database::MySQLPreparedStatement::AssertValidIndex(uint8 index)
{
    ASSERT(index < _paramCount || ParamenterIndexAssertFail(_stmt->GetIndex(), index, _paramCount));

    if (_paramsSet[index])
        LOG_ERROR("sql.sql", "[ERROR] Prepared Statement (id: {}) trying to bind value on already bound index ({}).", _stmt->GetIndex(), index);
}

template<typename T>
void Warhead::Database::MySQLPreparedStatement::SetParameter(const uint8 index, T value)
{
...
void Warhead::Database::MySQLPreparedStatement::SetParameter(uint8 index, bool value)
{
    SetParameter(index, uint8(value ? 1 : 0));
}

void Warhead::Database::MySQLPreparedStatement::SetParameter(uint8 index, std::nullptr_t /*value*/)
{
    AssertValidIndex(index);
    _paramsSet[index] = true;
    MYSQL_BIND* param = &_bind[index];
...
void Warhead::Database::MySQLPreparedStatement::SetParameter(uint8 index, std::string const& value)
{
    AssertValidIndex(index);
    _paramsSet[index] = true;
    MYSQL_BIND* param = &_bind[index];
...
void Warhead::Database::MySQLPreparedStatement::SetParameter(uint8 index, std::vector<uint8> const& value)
{
    AssertValidIndex(index);
    _paramsSet[index] = true;
    MYSQL_BIND* param = &_bind[index];
...
std::string Warhead::Database::MySQLPreparedStatement::getQueryString() const
{
