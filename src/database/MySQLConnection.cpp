...
namespace
{
    constexpr std::string GetConnectionFlagString(Warhead::Database::ConnectionFlags flag)
    {
        using enum Warhead::Database::ConnectionFlags;
...
Warhead::Database::MySQLConnectionInfo::MySQLConnectionInfo(std::string_view infoString)
{
...
Warhead::Database::MySQLConnection::MySQLConnection(MySQLConnectionInfo& connInfo, bool isAsync /*= false*/) :
...
Warhead::Database::MySQLConnection::~MySQLConnection()
{
    Close();
    LOG_WARN("sql.driver", "> {}", __FUNCTION__ );
}

void Warhead::Database::MySQLConnection::Close()
{
    if (_mysqlHandle)
    {
...
uint32 Warhead::Database::MySQLConnection::Open()
{
    auto mysqlInit = mysql_init(nullptr);
    if (!mysqlInit)
    {
        LOG_ERROR("sql.driver", "Could not initialize Mysql connection to database `{}`", _connectionInfo.Database);
    ...
    if (_mysqlHandle)
    {
        LOG_INFO("sql.sql", "Connected to MySQL database at {}", _connectionInfo.Host);
        mysql_autocommit(_mysqlHandle, 1);

        // set connection properties to UTF8 to properly handle locales for different
        // server configs - core sends data in UTF8, so MySQL must expect UTF8 too
        mysql_set_character_set(_mysqlHandle, "utf8mb4");
        return 0;
    }
    else
    {
        LOG_ERROR("sql.driver", "Could not connect to MySQL database at {}: {}", _connectionInfo.Host, mysql_error(mysqlInit));
...
bool Warhead::Database::MySQLConnection::Execute(std::string_view sql)
{
...

    {
        ...
        if (mysql_query(_mysqlHandle, sql.data()))
        {
            ...
            LOG_ERROR("sql.sql", "[{}] {}", err, mysql_error(_mysqlHandle));
...
bool Warhead::Database::MySQLConnection::Execute(PreparedStatement* stmt)
{
    if (!_mysqlHandle)
        return false;
    ...
    std::lock_guard<std::mutex> guard(_psMutex);
    ...
    {
        uint32 err = mysql_errno(_mysqlHandle);
        LOG_ERROR("sql.sql", "SQL(p): {}", mStmt->getQueryString());
        LOG_ERROR("sql.sql", "[{}] {}", err, mysql_stmt_error(msql_STMT));
    ...
    LOG_DEBUG("sql.sql", "[{}] SQL(p): {}", sw, mStmt->getQueryString());

    mStmt->ClearParameters();
    return true;
}

Warhead::Database::ResultSet* Warhead::Database::MySQLConnection::Query(std::string_view sql)
{
    if (sql.empty())
        return nullptr;
    ...
    return new Warhead::Database::ResultSet(result, fields, rowCount, fieldCount);
}

Warhead::Database::PreparedResultSet* Warhead::Database::MySQLConnection::Query(PreparedStatement* stmt)
{
    ...
    if (!Query(stmt, &mysqlStmt, &result, &rowCount, &fieldCount))
        return nullptr;
...
bool Warhead::Database::MySQLConnection::Query(std::string_view sql, MySQLResult** result, MySQLField** fields, uint64* rowCount, uint32* fieldCount)
{
    if (!_mysqlHandle || sql.empty())
        return false;
...
bool Warhead::Database::MySQLConnection::Query(PreparedStatement* stmt, MySQLPreparedStatement** mysqlStmt, MySQLResult** result, uint64* rowCount, uint32* fieldCount)
{
    if (!_mysqlHandle)
        return false;
    ...
    *mysqlStmt = mStmt;
    ...
    {
        uint32 err = mysql_errno(_mysqlHandle);
        LOG_ERROR("sql.sql", "SQL(p): {}\n [ERROR]: [{}] {}", mStmt->getQueryString(), err, mysql_stmt_error(msql_STMT));
...
/*static*/ std::string_view Warhead::Database::MySQLConnection::GetClientInfo()
{
    return { mysql_get_client_info() };
}

std::string_view Warhead::Database::MySQLConnection::GetServerInfo()
{
    return { mysql_get_server_info(_mysqlHandle) };
}

bool Warhead::Database::MySQLConnection::HandleMySQLError(uint32 errNo, uint8 attempts /*= 5*/)
{
    switch (errNo)
    {
        ...
        case CR_CONN_HOST_ERROR:
        {
            ...
            if (!lErrno)
            {
                ...
                LOG_INFO("sql.sql", "Successfully reconnected to {} @{}:{} Connection flags: {}.",
...
bool Warhead::Database::MySQLConnection::PrepareStatements() const
{
    return !_prepareError;
}

Warhead::Database::MySQLPreparedStatement* Warhead::Database::MySQLConnection::GetPreparedStatement(uint32 index)
{
...
void Warhead::Database::MySQLConnection::PrepareStatement(uint32 index, std::string_view sql, Warhead::Database::ConnectionFlags flags)
{
    ...
    if (!stmt)
    {
        ...
        LOG_ERROR("sql.sql", "{}", mysql_error(_mysqlHandle));
        _prepareError = true;
    }
    else
    {
