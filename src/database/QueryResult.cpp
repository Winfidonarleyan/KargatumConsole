...
namespace
{
    ...
    Warhead::Database::DatabaseFieldTypes MysqlTypeToFieldType(enum_field_types type)
    {
    ...
    void InitializeDatabaseFieldMetadata(Warhead::Database::QueryResultFieldMetadata* meta, MySQLField const* field, uint32 fieldIndex)
    {
...
Warhead::Database::ResultSet::ResultSet(MySQLResult* result, MySQLField* fields, uint64 rowCount, uint32 fieldCount) :
    _rowCount(rowCount),
    ...
    _fields(fields)
{
    ...
    for (uint32 i = 0; i < _fieldCount; i++)
    {
        ...
        field->SetMetadata(&_fieldMetadata[i]);
        _currRow.emplace(i, std::move(field));
    }
}

Warhead::Database::ResultSet::~ResultSet()
{
    CleanUp();
}

bool Warhead::Database::ResultSet::NextRow()
{
    if (!_result)
        return false;
...
std::string Warhead::Database::ResultSet::GetFieldName(uint32 index) const
{
    ASSERT(index < _fieldCount);
    return _fields[index].name;
}

void Warhead::Database::ResultSet::CleanUp()
{
    if (_result)
    {
...
Warhead::Database::Field const* Warhead::Database::ResultSet::operator[](std::size_t index) const
{
    ASSERT(index < _fieldCount);
    return _currRow.at(index).get();
}

void Warhead::Database::ResultSet::AssertRows(std::size_t sizeRows) const
{
    ASSERT(sizeRows == _fieldCount);
}

Warhead::Database::PreparedResultSet::PreparedResultSet(MySQLStmt* stmt, MySQLResult* result, uint64 rowCount, uint32 fieldCount) :
    _rowCount(rowCount),
    _fieldCount(fieldCount),
    _stmt(stmt),
    _metadataResult(result)
{
    ...
    {
        LOG_WARN("sql.sql", "{}:mysql_stmt_store_result, cannot bind result from MySQL server. Error: {}", __FUNCTION__, mysql_stmt_error(_stmt));
        delete[] _rBind;
        delete[] isNull;
        delete[] length;
        return;
    ...
    {
        LOG_WARN("sql.sql", "{}:mysql_stmt_bind_result, cannot bind result from MySQL server. Error: {}", __FUNCTION__, mysql_stmt_error(_stmt));
...
Warhead::Database::PreparedResultSet::~PreparedResultSet()
{
    CleanUp();
}

bool Warhead::Database::PreparedResultSet::NextRow()
{
...
bool Warhead::Database::PreparedResultSet::_NextRow()
{
...
Warhead::Database::Field* Warhead::Database::PreparedResultSet::Fetch() const
{
    ASSERT(_rowPosition < _rowCount);
    return const_cast<Field*>(&_rows[uint32(_rowPosition) * _fieldCount]);
}

Warhead::Database::Field const& Warhead::Database::PreparedResultSet::operator[](std::size_t index) const
{
    ASSERT(_rowPosition < _rowCount);
    ASSERT(index < _fieldCount);
...
void Warhead::Database::PreparedResultSet::CleanUp()
{
    if (_metadataResult)
...
void Warhead::Database::PreparedResultSet::AssertRows(std::size_t sizeRows) const
{
    ASSERT(_rowPosition < _rowCount);
