...
#define Crash(message) \
    ...
    RaiseException(0xC0000420L, 0, 2, execeptionArgs);
#else
...
namespace Warhead
{
    ...
    */
        ...
        std::string_view message, std::string const& fmtMessage = "", std::string const& debugInfo = "")
    {
        ...
        if (!fmtMessage.empty())
        {
            msg.append(StringFormat("# Message '{}'\n", fmtMessage));
        }

        if (!debugInfo.empty())
        {
            msg.append(StringFormat("\n# Debug info: '{}'\n", debugInfo));
        }
    ...
    */
    ...
    void Assert(std::string_view file, int line, std::string_view function, std::string const& debugInfo, std::string_view message, std::string const& fmtMessage /*= ""*/)
    {
    ...
    void Fatal(std::string_view file, int line, std::string_view function, std::string_view message, std::string const& fmtMessage /*= ""*/)
    {
    ...
    void Error(std::string_view file, int line, std::string_view function, std::string_view message)
    {
    ...
    void Warning(std::string_view file, int line, std::string_view function, std::string_view message)
    {
        std::string formattedMessage = MakeMessage("WARNING", file, line, function, message);
        fprintf(stderr, "%s", formattedMessage.c_str());
    }

    void Abort(std::string_view file, int line, std::string_view function, std::string const& fmtMessage /*= ""*/)
    {
